...
if (fs.existsSync(statePath)) {
	const { root: oldRoot, record: oldRecord } = JSON.parse(
	...
	if (root === oldRoot) {
		...
		const computeDiff = (old, current) => {
			...
			for (const key of onlyOldKeys) {
				...
				if (typeof value === "number")
					diff.push({ path: [...currentPath], mode: "Removed" });
			...
			for (const key of onlyCurrentKeys) {
				...
				if (typeof value === "number")
					diff.push({ path: [...currentPath], mode: "New" });
				else computeDiff({}, value);
				currentPath.pop();
			}
			for (const key of sharedKeys) {
				...
				if (typeof oldValue === "number" && typeof currentValue === "number") {
					if (oldValue > currentValue)
						diff.push({ path: [...currentPath], mode: "Shrunk" });
					else if (oldValue < currentValue)
						diff.push({ path: [...currentPath], mode: "Grown" });
		...
		while (diff.length) {
			...
			for (index = 0; index < itemPath.length - 1; index++)
				if (index > lastPath.length || lastPath[index] !== itemPath[index])
					break;
