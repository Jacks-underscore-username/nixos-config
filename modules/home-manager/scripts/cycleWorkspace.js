...
const targetId =
  monitorWorkspaces[
    (monitorWorkspaces.indexOf(focusedWorkspace) + Number.parseInt(process.argv[2]) + monitorWorkspaces.length) %
      monitorWorkspaces.length
  ]

if (targetId !== focusedWorkspace) execSync(`hyprctl dispatch workspace ${targetId}`)
