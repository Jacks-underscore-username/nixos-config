...
stdenv.mkDerivation (finalAttrs: {
  ...
  buildInputs = [love] ++ lib.optional withMods pkgs.unstable.lovely-injector;
  dontUnpack = true;
  desktopItems = [
    (makeDesktopItem {
      name = "balatro";
  ...
  installPhase = ''
    ...
    makeWrapper $out/share/Balatro $out/bin/balatro ${lib.optionalString withMods "--prefix LD_PRELOAD : '${pkgs.unstable.lovely-injector}/lib/liblovely.so'"}
    runHook postInstall
  '';
